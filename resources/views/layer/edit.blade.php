...
<div class="card-header py-2 h-body">
    <div class="row">
        <div class="col-md-6 pt-1">
            <h6 class="m-0 font-weight-bold text-primary">Edit Layer</h6>
        </div>
...
<div class="card-body h-body">
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{url('layer/'.$res->id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="row">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" class="form-control form-control-sm" value="{{ old('court_address')??$res->court_address }}" placeholder="Enter Address" name="court_address">
                        @error('court_address')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="text" class="form-control form-control-sm" value="{{ old('layer_name')??$res->layer_name }}" placeholder=" Enter Layer Name" name="layer_name">
                        @error('layer_name')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-4">
                        <label>Phone</label>
                        <input type="text" class="form-control form-control-sm" value="{{ old('phone')??$res->phone }}" placeholder=" Enter Phone" name="phone">
                        @error('phone')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control form-control-sm"  placeholder="Enter Client Name" name="client_id">
                            ...
                            <option value="{{ $show->id }}" {{($show->id==$res->client_id)?"selected":''}}>
                                {{ ucwords($show->share_holder)}}</option>
